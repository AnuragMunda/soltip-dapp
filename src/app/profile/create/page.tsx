...
const CreateProfile = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center gap-10 mt-10">
      <h1 className="text-3xl font-bold tracking-wide">Enter your details</h1>
      <Form action={handleCreateProfile} className="flex flex-col gap-10 h-full w-[50%]">
        <span>
          <Label about="name" className="mb-5 font-semibold tracking-wider text-lg">Full name</Label>
          <Input type="text" id="name" name="name" className="h-12" />
        </span>
        <span>
          <Label about="email" className="mb-5 font-semibold tracking-wider text-lg">Email</Label>
          <Input type="email" id="email" name="email" className="h-12" />
        </span>
        <span>
          <Label about="bio" className="mb-5 font-semibold tracking-wider text-lg">What are you creating?</Label>
          <Input type="text" id="bio" name="bio" placeholder="Bio" className="h-12" />
        </span>
        <span>
          <Label about="aboutMe" className="mb-5 font-semibold tracking-wider text-lg">About me</Label>
          <textarea className="border-2 w-full h-40 p-2 rounded-xl" id="aboutMe" name="aboutMe" placeholder="Something about you..." />
        </span>
        <Button className="py-6 rounded-full text-md" disabled={initializeProfile.isPending}>
          {initializeProfile.isPending ? `Processing...` : `Create your profile`}
        </Button>
      </Form>
    </div>
  )
